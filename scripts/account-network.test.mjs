...
function harness(user,rows=[],fail=false){
 const queries=[]
 const GET=Function('NextResponse','currentUser','prisma','canViewFullNetwork','Prisma','OWN_IDS','PAID',source.replace(/^import .*$/gm,'').replace(/export /g,'')+';return GET')(
  {json:(body,options)=>({body,...options})},async()=>user,
  {$queryRaw:async(strings,...values)=>{queries.push({sql:strings.join('?'),values});if(fail)throw Error('offline');return rows}},canViewFullNetwork,Prisma,OWN_IDS,PAID)
 return {GET,queries}
}
...
test('only the two fixed founder identities get recursive data and revenue',async()=>{
 for(const id of ['1f0ff6f6-47f6-433f-9ac3-137473003af2','4b829d44-616b-4dff-b5d6-eded9035874d']){
 const h=harness({id});const r=await h.GET();assert.equal(r.status,200);assert.equal(h.queries.length,2);assert.match(h.queries[0].sql,/WITH RECURSIVE/);assert.match(h.queries[0].sql,/ANY\(n.path\)/);assert.ok(h.queries[0].values.every(v=>v===id));assert.equal(r.body.totalRevenue,0);assert.match(h.queries[1].sql,/EXISTS/);assert.match(h.queries[1].sql,/NOT EXISTS/);assert.match(h.queries[1].sql,/IS NULL/)
 }
})
test('names, roles, query flags and impersonation do not grant founder access',async()=>{
 ...
 assert.equal(canViewFullNetwork(user),false);const h=harness(user);const r=await h.GET();assert.equal(h.queries.length,1);assert.equal('totalRevenue' in r.body,false);assert.doesNotMatch(h.queries[0].sql,/RECURSIVE|Order/)
 }
