...
export async function stopImpersonation(db,jar){
 const token=jar.get(impersonationCookie)?.value
 // Returning to the administrator must remain possible when the audit store is unavailable.
 // Clear only the impersonation cookie; keep the administrator's auth session intact.
 jar.set(impersonationCookie,'',{path:'/',maxAge:0,httpOnly:true,sameSite:'strict',secure:process.env.NODE_ENV==='production'})
 if(!token)return
 try{await db.$executeRaw`UPDATE public."ElyseraImpersonationSession" SET "endedAt"=NOW() WHERE "tokenHash"=${tokenHash(token)} AND "endedAt" IS NULL`}
 catch{console.error('Impersonation cookie cleared, but session end could not be recorded; database expiry still applies.')}
}
