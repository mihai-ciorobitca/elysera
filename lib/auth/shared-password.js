...
export async function setSharedPassword(user,password,db=prisma){
 ...
 const write=async tx=>{
  ...
  // Password recovery/change must also revoke refresh sessions. The provider's
  // refresh_tokens.session_id foreign key cascades when sessions are removed.
  await tx.$executeRaw`DELETE FROM auth.sessions WHERE user_id=${user.supabaseUserId}::uuid`
  return true
 }
