...
export async function activateRegistration(email,password,db=prisma){
 const activate=async tx=>{
  const rows=await tx.$queryRaw`SELECT u.id,u.email,u.password,u."supabaseUserId",u."emailVerified",u.blocked,u.role::text AS role,u."emailDeliveryStatus"::text AS "emailDeliveryStatus",a.email AS "authEmail",a.banned_until,
  ...
  if(!/^\$2[aby]\$[0-9]{2}\$[./A-Za-z0-9]{53}$/.test(user.password||'')||!await bcrypt.compare(password,user.password))return false;
