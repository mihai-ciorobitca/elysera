...
export async function prepareLegacyPassword(email,password){
 return prisma.$transaction(async tx=>{
  const users=await tx.$queryRaw`SELECT "id","email","password","supabaseUserId","emailVerified","blocked","role"::text AS role FROM public."User" WHERE lower("email")=${email} LIMIT 2 FOR UPDATE`
  if(users.length!==1)return null
  const user=users[0]
  if(!bcryptHash.test(user.password||'')||['ADMIN','STAFF'].includes(user.role))return null
  if(user.blocked)return denied
  if(!await bcrypt.compare(password,user.password))return denied
  if(!user.emailVerified)return {code:'email_not_confirmed',status:400}
  ...
  if(identity){
   ...
   if(!identity.email_confirmed_at)return {code:'email_not_confirmed',status:400}
   ...
   if(identity.encrypted_password!==user.password)await tx.$executeRaw`UPDATE auth.users SET encrypted_password=${user.password},updated_at=NOW() WHERE id=${identity.id}::uuid`
  }else{
