...
async function resolvePrincipal(client,admin=false){
 ...
 const user=rows[0]
 if(admin){const access=await prisma.$queryRaw`SELECT "enabled" FROM public."ElyseraAdminAccess" WHERE "userId"=${user.id}`;if(!eligibleAdmin(identity,user,access[0]?.enabled===true))return null;return {...user,role:'ADMIN'}}
