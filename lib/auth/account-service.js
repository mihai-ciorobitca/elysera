...
export async function registerAccount(email,password,name,last){requireMail();const existing=await findAccount(email);if(existing){if(allowedAccount(existing)&&!existing.emailVerified)await issueToken(existing,'verify');return}
 ...
 try{await prisma.$executeRaw`INSERT INTO "User" ("id","email","firstName","secondName","password","supabaseUserId","emailVerified","updatedAt") VALUES (${id},${email},${name},${last},${'!supabase-only:'+randomUUID()},${identity.id}::uuid,false,NOW())`}
