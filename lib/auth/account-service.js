...
export async function registerAccount(email,password,name,last,details,referral){const existing=await findAccount(email);if(existing){return {ready:Boolean(allowedAccount(existing)&&!existing.emailVerified&&await activateRegistration(email,password)),created:false}}
 const admin=adminClient();const {data,error}=await admin.auth.admin.createUser({email,password,email_confirm:false});if(error){if(['email_exists','user_already_exists'].includes(error.code))return {ready:false,created:false};throw Error('REGISTRATION_FAILED')}
 ...
 if(!await activateRegistration(email,password))throw Error('REGISTRATION_ACTIVATION_FAILED')
