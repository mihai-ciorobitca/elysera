...
export async function POST(request){
 ...
 try{
  ...
  if(!await adminPrincipalFor(client)){await client.auth.signOut({scope:'local'});return reply({error:'Dieses Konto kann hier nicht angemeldet werden.'},403)}
  return reply({ok:true,redirect:'/admin'})
