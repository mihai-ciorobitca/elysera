...
export async function GET(){
 try{
  ...
  const commissions=await prisma.$queryRaw`SELECT c."id",c."orderId",c."amount",c."status"::text AS status,c."createdAt",c."holdUntil" FROM public."Commission" c WHERE c."userId"=${user.id} AND c."orderId" IS NOT NULL AND EXISTS (SELECT 1 FROM public."OrderItem" i WHERE i."orderId"=c."orderId" AND i."productId" IN (${Prisma.join(ids)})) AND NOT EXISTS (SELECT 1 FROM public."OrderItem" other WHERE other."orderId"=c."orderId" AND (other."productId" IS NULL OR other."productId" NOT IN (${Prisma.join(ids)}))) ORDER BY c."createdAt" DESC LIMIT 200`
