...
export async function GET(request){
 try{
  ...
  const result=await prisma.$transaction(async tx=>{
   const accounts=await tx.$queryRaw`SELECT u."id",u."firstName",u."secondName",u."email",p."updatedAt",p."firstLoginAt" FROM public."ElyseraAccountProfile" p JOIN public."User" u ON u."id"=p."userId" ORDER BY p."updatedAt" DESC,u."id"`;
   ...
   const relationships=table[0]?.ready?await tx.$queryRaw`SELECT r."userId",r."parentUserId",r."status" FROM public."ElyseraPartnerProfile" r JOIN public."ElyseraAccountProfile" a ON a."userId"=r."userId"`:[];
