...
export async function POST(request,{params}){
 ...
 if(action==='register'){const details=validateDetails(b);if(!details)return reply({error:'Bitte vollständigen Namen und Adresse eingeben.'},400);if(typeof b.name!=='string'||!b.name.trim()||b.name.length>80||typeof b.last!=='string'||b.last.length>80)return reply({error:'Bitte deinen Namen eingeben.'},400);await registerAccount(email,b.password,b.name.trim(),b.last.trim(),details,b.referral);return reply({message:'Falls dein Konto noch bestätigt werden muss, erhältst du eine E-Mail. Bei einem bestehenden Konto kannst du dich anmelden oder dein Passwort zurücksetzen.'})}
