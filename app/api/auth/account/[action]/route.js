...
export async function POST(request,{params}){
 ...
 if(action==='register'){const details=validateDetails(b);if(!details)return reply({error:'Bitte vollständigen Namen und Adresse eingeben.'},400);if(typeof b.name!=='string'||!b.name.trim()||b.name.length>80||typeof b.last!=='string'||b.last.length>80)return reply({error:'Bitte deinen Namen eingeben.'},400);const result=await registerAccount(email,b.password,b.name.trim(),b.last.trim(),details,b.referral);return reply({ready:result.ready,message:result.ready?'Dein Konto ist bereit. Du kannst dich jetzt anmelden.':'Für diese E-Mail-Adresse besteht bereits ein Konto. Melde dich mit deinem bestehenden Passwort an oder nutze „Passwort vergessen?“. '})}
