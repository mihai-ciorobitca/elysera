...
export function CampaignImage({name,alt='',className='',priority=false,sizes='(max-width: 600px) 100vw, 50vw',animate=true}){
 if(name.startsWith('usage-'))return <img className={className} src={`/media/product-application/${name.slice(6)}${['usage-toner-2','usage-serum-3','usage-eye-1','usage-eye-2'].includes(name)?'-v2':''}.webp`} alt={alt} loading={priority?'eager':'lazy'} decoding="async"/>
...
export function Gallery({product:p}){
 ...
 const images=[{name:key,label:'Produkt',alt:`${p.name}, ${p.volume}`},...labels.map((label,i)=>({name:`usage-${key}-${i+1}`,label,alt:`${p.short}: ${label} – illustrative Anwendung`})).filter(im=>im.name!=='usage-serum-1')]
