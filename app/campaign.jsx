...
export const mediaRoot='/media/campaign-2026/'
export function CampaignImage({name,alt='',className='',priority=false,sizes='(max-width: 600px) 100vw, 50vw'}){
 return <img className={className} src={`${mediaRoot}${name}.webp`} srcSet={`${mediaRoot}${name}-small.webp 560w, ${mediaRoot}${name}.webp ${name==='hero'?1672:1122}w`} sizes={sizes} alt={alt} loading={priority?'eager':'lazy'} fetchPriority={priority?'high':'auto'} decoding="async"/>
}
export function CampaignMotion(props){return <ScrollMedia name={props.name}><CampaignImage {...props}/></ScrollMedia>}
