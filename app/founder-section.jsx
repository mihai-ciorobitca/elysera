...
export default function FounderSection({person='bettina'}){
 ...
 const portraits=isJessica?['confident']:['laugh','wink','confident']
 const expressions=isJessica?['im dunkelblauen Anzug mit heller Bluse']:['lachend, seitlich im Sessel','mit einem Augenzwinkern','stehend mit verschränkten Armen']
 const ref=useRef(null),touch=useRef(null),[active,setActive]=useState(0)
 const next=()=>setActive(i=>(i+1)%portraits.length)
 ...
 return <section className={`founder-section${isJessica?' jessica-founder':''}`} id={sectionId} aria-labelledby={titleId}>
  <div className="founder-portraits" ref={ref} aria-label={`Porträts von ${name}`} aria-roledescription="Karussell" onTouchStart={e=>{touch.current=e.touches[0].clientX}} onTouchEnd={e=>{if(touch.current!==null){const delta=e.changedTouches[0].clientX-touch.current;if(Math.abs(delta)>40)setActive(i=>(i+(delta<0?1:portraits.length-1))%portraits.length)}touch.current=null}}>
   <div className="founder-frames">{portraits.map((n,i)=><img key={n} src={isJessica?`/media/jessica/portrait-${n}-1122.webp`:`/media/bettina/portrait-${n}.webp`} width="1122" height="1402" loading="lazy" decoding="async" alt={active===i?`${name} – ${expressions[i]}`:''} aria-hidden={active!==i} className={active===i?'is-active':''}/>)}</div>
   {portraits.length>1&&<div className="founder-controls"><button aria-label="Vorheriges Porträt" onClick={()=>setActive(i=>(i+portraits.length-1)%portraits.length)}>←</button><div>{portraits.map((_,i)=><button key={i} aria-label={`Porträt ${i+1} anzeigen`} aria-pressed={active===i} onClick={()=>setActive(i)}><span/></button>)}</div><button aria-label="Nächstes Porträt" onClick={next}>→</button></div>}
  </div>
