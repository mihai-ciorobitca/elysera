...
export default async function Page({searchParams}){const params=await searchParams;if(process.env.NODE_ENV==='development'&&params?.preview==='design')return <SignIn/>;let user,initialMfa=false;try{user=await currentUser();if(!user){const client=await authClient();const {data}=await client.auth.getUser();if(data.user){const {data:a}=await client.auth.mfa.getAuthenticatorAssuranceLevel();initialMfa=a?.nextLevel==='aal2'&&a.currentLevel!=='aal2'}}}catch{}if(user)redirect('/dashboard');return <SignIn initialMfa={initialMfa} initialError={googleErrors[params?.google]||''}/>}
