...
export default function LoginGallery(){
 const [active,setActive]=useState(0),[reduced,setReduced]=useState(true)
 ...
 return <section className="es-login-gallery" aria-label="ELYSERA Bildergalerie">
  {slides.map((slide,i)=><div className={`es-gallery-slide ${active===i?'is-active':''}`} aria-hidden={active!==i} key={slide.src}><img src={'/media/login-lifestyle/'+slide.src} alt={slide.alt} style={{objectPosition:slide.position}} fetchPriority={i===0?'high':'auto'}/><div className="es-gallery-copy"><h2>{slide.title}</h2><p>{slide.text}</p></div></div>)}
