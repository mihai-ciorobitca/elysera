...
export function Shell({children}){
 ...
 {(panel==='menu'||panel==='discover')&&<nav className="mobile-links" aria-label="Hauptnavigation">{navGroups.map(([title,links])=><div className="mobile-nav-group" key={title}><h3>{title}</h3>{links.map(([href,label])=><Link key={href} href={href} onClick={()=>setPanel(null)}>{label}<Icon/></Link>)}</div>)}</nav>}
...
export function RoutineQuiz(){
 ...
 const questions=[{title:'WAS STEHT FÜR DICH IM FOKUS?',options:['Feuchtigkeit & Vorbereitung','Gezielte Peptidpflege','Die Augenpartie']},{title:'WIE MÖCHTEST DU STARTEN?',options:['Mit einem gezielten Produkt','Mit der gesamten Routine']}]
 const routine=['balance-toner','renewal-serum','contour-eye-cream'].map(slug=>products.find(p=>p.slug===slug)).filter(Boolean)
 const result=answers[1]===1?routine:[routine[answers[0]]].filter(Boolean)
 const current=questions[step]
 return <section className="section quiz-section"><div className="page-heading"><h1>DEINE HAUT.<br/><em>DEINE ROUTINE.</em></h1><p>Zwei kurze Fragen helfen dir, deinen Einstieg in die Kollektion zu finden.</p></div>{step<questions.length?<div className="quiz-body"><div className="quiz-progress" aria-label={`Frage ${step+1} von ${questions.length}`}>{questions.map((_,i)=><span key={i} className={i<=step?'active':''}/>)}</div><span className="eyebrow">FRAGE {step+1} VON {questions.length}</span><h2 ref={questionHeading} tabIndex={-1}>{current.title}</h2><div className="quiz-options">{current.options.map((option,i)=><button key={option} aria-pressed={answers[step]===i} onClick={()=>setAnswers(a=>{const next=[...a];next[step]=i;return next})}><span className="radio-dot"/>{option}<Icon/></button>)}</div><div className="quiz-navigation"><button className="text-link" disabled={step===0} onClick={()=>setStep(s=>s-1)}>← ZURÜCK</button><button className="button" disabled={answers[step]===undefined} onClick={()=>setStep(s=>s+1)}>{step===questions.length-1?'MEINE ROUTINE ANSEHEN':'WEITER'}<Icon/></button></div></div>:<div className="quiz-result"><h2 ref={questionHeading} tabIndex={-1}>{result.length===3?'DEINE ROUTINE IN DREI SCHRITTEN':'DEIN EINSTIEG IN DIE PEPTIDPFLEGE'}</h2><p>{result.length===3?'Beginne mit dem Toner, ergänze das Serum und schließe mit der Augenpflege ab.':`Passend zu deinem Schwerpunkt: ${result[0].name}.`}</p><div className={`product-grid ${result.length===1?'single-result':''}`}>{result.map(p=><ProductCard key={p.slug} product={p}/>)}</div><button className="text-link" onClick={()=>{setStep(0);setAnswers([])}}>QUIZ NEU STARTEN ↻</button></div>}<p className="source-note">Orientierung innerhalb der ELYSERA Kollektion. Deine Antworten werden nicht übertragen oder gespeichert.</p></section>
}
export function TextureFilms(){return <TextureLibrary/>}

export function RoutineSet({label="3ER-SET · 139 € VORMERKEN"}){const {addRoutine,ready}=useContext(Cart);return <button className="button routine-set-button" disabled={!ready} onClick={addRoutine}><span>{label}</span><Icon name="bag"/></button>}
